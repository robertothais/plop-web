(function() {
  var Util, url;

  url = require('url');

  Util = (function() {

    function Util() {}

    return Util;

  })();

  Util.prototype.calculateScore = function(date, votes) {
    var now, order, rawScore, seconds;
    now = Math.round(date.getTime() / 1000);
    seconds = now - 1331323254;
    order = Math.log(votes) / Math.log(10);
    rawScore = order + seconds / 45000;
    return Math.round(rawScore * 10000000) / 10000000;
  };

  Util.prototype.randomString = function(length) {
    var i, out, rnum;
    out = "";
    i = 0;
    while (i < length) {
      rnum = Math.floor(Math.random() * this.chars.length);
      out += this.chars.substring(rnum, rnum + 1);
      i++;
    }
    return out;
  };

  Util.prototype.parseAssembly = function(assembly) {
    var out, results, size, upload;
    results = JSON.parse(assembly).results;
    out = {};
    ['medium', 'large'].forEach(function(size) {
      return out[size] = results["resize_" + size][0];
    });
    if (parseInt(out.large.meta.width) <= parseInt(out.medium.meta.width)) {
      out.large = out.medium;
    }
    for (size in out) {
      upload = out[size];
      out[size] = url.parse(upload.url).pathname.substring(1);
    }
    return out;
  };

  Util.prototype.chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghijklmnopqrstuvwxyz";

  module.exports = Util;

}).call(this);
